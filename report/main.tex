...
\abstract{This paper presents a reproducible deep learning workflow for classifying
  ...
  curbs the overfitting observed when training ran a fixed 20 epochs. On the
  shared validation split, the CNN achieved .826 accuracy, exceeding the
  LSTM's .743. Both models are selected only with validation performance; the
  test split remains reserved for final evaluation.}
...
split. The CNN was the stronger validation model, improving accuracy by 8.3
percentage points and weighted F1 by 8.5 percentage points over the LSTM.
...
\begin{table}[htbp]
  ...
  \begin{threeparttable}
    ...
    \begin{tabular}{@{}lrrrrl@{}}
      ...
      \midrule
      LSTM baseline & 6 & .743 & .738 & .606 & Reserved \\
      CNN baseline & 9 & .826 & .823 & .740 & Reserved \\
      \bottomrule
    \end{tabular}
    \tablenote{Metrics are from the validation-selected, early-stopped checkpoints
      trained with weight-decayed Adam on the fixed clean manifest. The test
      split was not evaluated.}
  \end{threeparttable}
\end{table}

Figure~\ref{fig:loss-curves} shows that both models triggered early stopping
well before the 20-epoch budget (LSTM at epoch 11, CNN at epoch 14), which
supports checkpoint selection rather than using the final epoch. At its
selected checkpoint, the CNN's training accuracy (82.8\%) nearly matches its
validation accuracy (82.6\%), a far smaller generalization gap than the
unregularized run. Figures~\ref{fig:lstm-matrix} and \ref{fig:cnn-matrix} show
the class-level outcome. The CNN improved recall for Chopin (.800 versus .600)
and Beethoven (.677 versus .323), while both models continued to underperform
on Beethoven and Mozart relative to Bach.
...
architectures while validation loss stagnated or worsened, which would have
inflated any eventual test-set claim. Selecting checkpoints from a
regularized, early-stopped run gives a more defensible estimate of
generalization before the test split is finally opened.
